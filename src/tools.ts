...
export function registerJenkinsTools(server: McpServer, context: ToolContext): void {
  ...
  server.registerTool(
    ...
    async ({ jobPath, build, start, maxChars }) =>
      toToolResult(await context.client.getConsoleLog(jobPath, build, { start, maxChars }), context.secrets)
  );
  ...
  server.registerTool(
    "jenkins.trigger_build",
    {
      ...
      description: "Trigger a Jenkins build. Disabled unless mutation settings and job allowlist permit it.",
      inputSchema: {
        jobPath: z.string().min(1),
  ...
  server.registerTool(
    "jenkins.stop_build",
    {
      ...
      description: "Stop a Jenkins build. Disabled unless mutation settings and job allowlist permit it.",
      inputSchema: {
        jobPath: z.string().min(1),
